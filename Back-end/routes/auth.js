...
router.post("/register", async (req, res) => {
  ...
  Admin code validation (if admin):
  Raw SQL:
  SELECT * FROM admin_codes WHERE code = $1 LIMIT 1

  User insertion:
  Raw SQL (for regular user):
  
    INSERT INTO users (email, password, phone, user_name, city)
    VALUES ($1, $2, $3, $4, $5)
    RETURNING user_id
 ...
 Admin insertion:
  Raw SQL (for admin):
    INSERT INTO admins (email, password, phone, admin_name, designation)
    VALUES ($1, $2, $3, $4, $5)
    RETURNING admin_id
  */
...
router.post("/login", async (req, res) => {
  ...
  /* Raw SQL for user check:
  SELECT * FROM users WHERE email = $1 LIMIT 1
  
  Raw SQL for admin check:
  SELECT * FROM admins WHERE email = $1 LIMIT 1 */
